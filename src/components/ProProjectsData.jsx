const proProjectsData = [
  {
    id: 1,
    category: "Back-End Development",
    title: "Manithy Omnichannel Internal Chatbot",
    description: "Developed scalable omnichannel B2B backend APIs and AI agents with Next.js, Mastra, Redis & TypeScript, integrated with MCP for intelligent workflow orchestration.",
    tech: ["Next.JS", "Mastra", "Prompt Develop", "Redis", "Swagger", "Javascript", "TypeScript", "Python", "MCP"],
    color: "from-yellow-500 to-amber-500",
    link: "",
    image: "../images/manithy_project.png"
  },
  {
    id: 2,
    category: "Mobile Development",
    title: "Manulife Mobile System",
    description: "Shipped a full Android app in just 7 days using Ionic & Angular — freelance work for PT RDS's Manulife tender bid.",
    tech: ["Ionic", "Angular.JS", "Javascript"],
    color: "from-yellow-500 to-amber-500",
    link: "",
    image: "../images/manulife_project.jpg"
  },
  {
    id: 3,
    category: "Full Stack Development",
    title: "Taspen Life Individu System",
    description: "End-to-end backend & frontend development — building REST APIs in C# with NHibernate ORM, backed by PostgreSQL, for business process in Taspen Life.",
    tech: [".NET", "Swagger", "Blazor", "NHibernate"],
    color: "from-purple-500 to-pink-500",
    link: "",
    image: "../images/taspen_project.png"
  },

];

export default proProjectsData;